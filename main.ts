...
schedule("0 * * * * *", async () => {
  ...
  if (
    !dhlResponse ||
    !dhlResponse.sendungen[0].sendungsdetails.sendungsverlauf
  ...
  if (await fileExists("lastUpdatedValue")) {
    const lastUpdatedValue: string = await Deno.readTextFile(
      "lastUpdatedValue",
    );
    if (lastUpdatedValue.trim() == updated.toISOString()) {
      ...
      const content: string = CONTENT_FORMAT_STRING.replace(
        "{0}",
        timestamp.toString(),
      ).replace("{1}", name).replace("{2}", trackingCode).replace(
        "{3}",
        message,
      )
        .replace("{4}", link);
